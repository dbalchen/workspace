...
int gy_ccr_send(int client_sock, unsigned int cc_type,unsigned int requested_action, std::string SessionID, std::string mdn, vector<DIAMETER_avp> avp_list, int32_t ccReq = 1) {
...
int gy_ccr_initial(int client_sock,std::string SessionID,std::string mdn, std::string amount) {
	...
	content_description_code_avp.setValue("AUTH");
	...
	purchase_category_code_avp.setValue("google-dev-content");
	...
	application_type_avp.setValue("One-Off");
...
int gy_ccr_terminal(int client_sock,std::string SessionID, std::string mdn, std::string amount, int32_t ccReq = 1) {
	...
	partner_id_avp.setValue("51143d511662200");
...
// Credit Account
		content_description_code_avp.setValue("CREDIT");
		requested_service_unit.setAvp(cc_money_avp);
		avp_list.push_back(requested_service_unit);
		avp_list.push_back(adjustment_reason_code);
...
// Debit account
		content_description_code_avp.setValue("DEBIT");
		used_service_unit.setAvp(cc_money_avp);
		avp_list.push_back(used_service_unit);
	}
