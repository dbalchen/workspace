...
-  
   ...
   -  /A successful man made system will closely resembles some natural
      system./
...
-  Usage Overview
   ...
   An event that gets rated in the system is called usage and is why we
   have a billing system in the first place. We tend to think of usage
   in two types, *Voice* and *Data*.
   ...
   *Voice*
   ...
   4. *CIBER* - For /InCollect and OutCollect/ processing we do not
      convert to *UFF*, instead the *CIBER* record format is used.

   *Data*

   The following switch types are first converted into the *UFF* CDR
   format:\\
...
-  + - Databases
   ...
   -  + -
      ...
      -  
         ...
         Similar to *ac\_processing\_accounting* there are two tables
      ...
      -  
         ...
         | *CYCLE\_INSTANCE*                    | NUMBER(2,0)           | Cycle Instance = 0    |
         |                                      |                       | Pre-Paid Subscriber   |
         ...
         | L9\_CUSTOMER\_TYPE                   | VARCHAR2(1 BYTE)      |                       |
         | L9\_CALLED\_COUNTRY                  | VARCHAR2(3 BYTE)      |                       |
         | *L9\_PAYMENT\_CATEGORY*              | VARCHAR2(4 BYTE)      | POST or PRE           |
      ...
      -  
         ...
         in sync.
         [[file:///home/dbalchen/workspace/Outcollects/updateSeq.pl][Sequence
         Creation Job]]
      ...
      -  
         ...
         template.
         [[file:///home/dbalchen/workspace/CommonPlace/docs/em1_example.sql][EM1\_RECORD
         Example]]
   ...
   -  + -
      ...
      *BPT* tables that we are responsible for:
      ...
      -  
         ...
         foreign carriers see the *VISITOR\_MIN\_LR*.
      ...
      -  
         ...
         This maybe another version of the *ADJ9\_TIME\_ZONE\_REF*
         table, very similar.
      ...
      -  + -
         ...
         -  
            ...
            A description of each *SID* found in the *PC9\_SID* table.
         ...
         -  
            ...
            | EFFECTIVE\_DATE                | DATE                 |                   |
            | *AIR\_TIME\_IND*               | CHAR(1 BYTE)         | N = Air Time      |
            |                                |                      | is free           |
            | TOLL\_SPECIAL\_NUMBER\_GROUP   | VARCHAR2(255 BYTE)   |                   |
            | *DROP\_CALL\_IND*              | CHAR(1 BYTE)         | Y = This record   |
            |                                |                      | Will be dropped   |
            | SPECIAL\_NUMBER\_TYPE          | CHAR(1 BYTE)         |                   |
            | SERVICE\_FILTER                | VARCHAR2(15 BYTE)    |                   |
            | *TOLL\_FREE\_IND*              | CHAR(1 BYTE)         | Y = No Toll       |
            |                                |                      | will be charged   |
         ...
         -  
            ...
            This table as well and *PC3\_SERVICE\_FILTER\_LIST* are used
            by the *RLC*.
         ...
         -  
            ...
            This table as well and *PC3\_SERVICE\_FILTER* are used by
            the *RLC*.
         ...
         -  + -
            ...
            -  
               ...
               1. Find the entry in the *PC9\_INCOL\_SID\_PAIR* where
               ...
               2. Create an Insert statement for the
                  *PC9\_INCOL\_SID\_PAIR* with the new rates.
               ...
               1. For each *SID* found add '175' to the end and use that
                  ...
                  *AGD1\_RESOURCES\_REF* then create an insert if it
                  don't exists.
               ...
               1. For each *SID* found add a '1' in front which will get
                  you the *customer\_id* then do a query against the
                  *APE1\_SUBSCR\_DATA\_REF* to get the subscriber\_id
               ...
               1. Once you have the subscriber you need to point each
                  ...
                  *APE1\_SUBSCR\_OFFERS\_REF* table to the correct air
                  and toll charge. /(Again using the above example)/ To
                  find a suitable offer ID search the *CSM\_OFFER*
   ...
   -  + -
      ...
      -  
         ...
         | AU\_PROD\_CAT\_ID     | CHAR (2 Byte)        |                  |
         | AU\_BP\_START\_DATE   | DATE                 |                  |
         | AU\_CHARGE            | NUMBER               |                  |
         | GL\_ACCOUNT           | VARCHAR2 (17 Byte)   |                  |
...
-  + - TOPS Operations
   ...
   -  
      ...
      *A&F* is to the new system what *MAF and RBMS* was to the old.
      ...
      from the *UFF* to the usage record^{[[#fn.1][1]]}. If an *A&F*
   ...
   -  + -
      ...
      [[Pictures/TC.png]] Though the overall architecture seems to be
      the same, with some name changes like *MAF* is now
      A&F^{[[#fn.2][2]]}, as *CARES* the major change has been the
      ...
      -  *Event flow:*
         ...
         3. Gets Rated
            ...
            -  For *Pre-Pay* the HLR^{[[#fn.3][3]]}. is handled by the
               *SCP*
      ...
      -  
         ...
         *APE1\_REJECTED\_EVENTS* table. For *A&F* they are in the
         *EM1\_RECORD* table. Since there are so many coulmns in the
         EM1\_RECORD table we must limit are query's to the following
         columns. [[docs/EM1%20Query's][EM1 Queries]]
   ...
   -  + -
      ...
      *CIBER* files are a collection of roaming records, these can be
      ...
      -  *OUTCOL*\\
         Extracts from the *APE1\_RATED\_EVENT* table and creates files
         for *MAS*.
      -  *ADJ9MAS OUTCOL*\\
   ...
   -  + -
      ...
      events are processed by *TC* and added to the *APE1\_ACCUMULATORS*
      table a check is made against the
      *L9\_FIRST\_THRESHOLD/L9\_SECOND\_THRESHOLD* fields. If an overage
   ...
   -  
      ...
      *ADJ3\_APR\_CycleBillRun\_Sh*. If it completes successfully it
      ...
      [[Pictures/billing_tc_map-27124108.png]]
   ...
   -  + -
      ...
      -  
         ...
         -  F2E | ssh prdwrk4@kpr02batch |
            ADJ1\_File2E\_Daemon\_Shell\_Sh\_F2E
   ...
   -  + -
      ...
      -  
         ...
         -  phy\_file\_ident = {some identifier}
            [[docs/Operational%20File%20Query][AC\_CONTROL\_HIST Query]]

      -  

         Incollect Voice CIBER

         [[docs/Ciber_Voice_Incollects.sql][Voice Operational SQL]]

      -  

         Incollect Data CIBER

         [[docs/Ciber_Data_Incollects.sql][Data Operational SQL]]

   -  

      TC Runbook

      [[docs/AB%208%201%20-%20TC%208%201%20SP2%20-%20Run%20Book%20Core.pdf][TC
      Runbook Version 8.1]]
      ...
      -  + -
         ...
         -  
            ...
            -  File location: ~aprmoper/var/usc/SAPGLEXTR/IN/
               ...
               -  Ex:
                  /inf\_nas/apm1/prod/aprmoper/var/usc/SAPGLEXTR/IN/GLINCY420130401005834
         ...
         -  
            ...
            -  File location: ~aprmoper/var/usc/SAPGLEXTR/RO/
               ...
               -  Ex:
                  /inf\_nas/apm1/prod/aprmoper/var/usc/SAPGLEXTR/RO/GLOUTY420130401005834
         ...
         -  
            ...
            -  File location: ~aprmoper/var/usc/SAPGLEXTR/IR/
               ...
               -  Ex:
                  /inf\_nas/apm1/prod/aprmoper/var/usc/SAPGLEXTR/IR/GLINTY320130401005834
   ...
   -  + - TC Oncall Daily Duties
      ...
      -  
         ...
         -  Go to the MPS mailbox and look for the AC1\_CONTROL Fixer
...
-  + - CallDump
   ...
   -  Data Directories
      ...
      -  */m04/switchb/ecs* - *(aaa1)* 3G or lower data usage guide by
         *#19*.
   ...
   -  
      ...
      1. Job PR-BOD-S2B\_TO\_WEDO is running this script:
         /m01/switch/to\_wedo.sh
      2. Job PR-BOD-S2B\_CREATE\_WEDO\_ARCH is running this script:
         /m01/switch/wedo/create\_wedo\_archive.sh
