...
** Create Wallets 						   :noexport:
 : Service Account SVC_NP_BILLINGOPS xDzEikowgeQLcjfoKHXsawuu$
 : Service Account SVC_BILLINGOPS    xDzEikowgeQLcjfoKHXsawuu$
 :
...
mkstore -wrl .wallet  -modifyCredential ANF_SVC_BILLINGOPS SVC_BILLINGOPS 'xDzEikowgeQLcjfoKHXsawuu$'
...
* Task List
** TODO This Weeks Tasks
   - Provided verification for all test calls for VoLTE wave 4.
   - Oncall all week thus participated in all operation meetings.
   - Provided Usage Graphs for Connie.
   - Finished the first Ifra-Ops transfer document for the CallDump
     - Rewrote and tested the shutdown CallDump script.
     - Started working on EOM document.
   - Did analysis on moving the ENODEB tower dump to the CallDump.
   - Reviewed and modified the Usage Playbook
...
**** Upcoming events.
    ...
    Monitor the CDR’s for charging validations and opportunity to look at the new 5G parameters to distinguish the usage based on the network raw CDR’s 
    ...
    *The project status is Off Track for Schedule of Reporting delivery in Q2 2020.*
     ...
     - Splitting 4g-5G Traffic - Ericsson dependency – Engineering team and leaders are aware.
     - ETQA and EPC team are engaged on the Production Test SKU issue. The fix will be in production 3/26 – Work around is in place in the meantime.
    ...
    *Phase 1 of 5G has been implimented and in production (Iowa and Wisconsin) and there have been no issues so far.* 
    Received a “go” decision from the executive team to launch our 5G devices. Began Live BMC on 03/04.
...
**** Next Steps:
     ...
     - Device manager is still working on getting a device list from Samsung for OEM testing – Due Date 04/17
     - Amdocs code delivery 05/11
     - ETQA Testing 05/11 – 05/31
...
*** Current Status
    - Action Items:
      ...
      5. Sarit – Confirm Logic can be updated to reference only the last 10 digits on the number to ignore for rating .
      6. Michelle – Confirm with the business if we expect using this number for International Roaming to impact the volume of calls. 
