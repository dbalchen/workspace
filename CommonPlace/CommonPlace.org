...
** Mable Jobs
   The following is a list of *EBI* jobs for *Consolidated Billing*. To run *[[http://isoc.uscc.com/supportrequestform.php][Complete an ISOC Request]]* with the instructions below:
    ...
    5. Additional Questions: *Run job – no paramenters needed*
...
| *Column Name*     | *Datatype*    | *Description*            |
|-------------------+---------------+--------------------------|
| Ba_Account_No     | Number (12)   | *FA* Finacial Account    |
...
*** Project Description
    ...
    - Network enablement in Year 1 markets (Iowa and Wisconsin)
...
*** Go Live Dates
    ...
    - Track1 – TOPS Provisioning changes (Amdocs) – 1 PI effort – 1 User story –  target August, 2019
    ...
    - Reporting Enablement for 5G will be part of future track of work as aligned with Business.
...
*** Status
    ...
    - Mr. Hauk has sent out the various proposels on the 5G usage which includes:
    ...
    - 5G Phase 2 Business requirements first draft is complete. This will cover reporting and data varification (We will have a bigger roll).
    - Solution Document for phase 1 has been updated.
    - Currently tracking the Track2 scoping timeline from Amdocs. Conducting weekly meetings with IS and Engineering teams 
      to look at lab readiness roadmap as well as initial UE samples that may become available for lab and/or field trials.
      Leveraging action-item repository in the enterprise sharepoint site for items we need addressed by IS and Engineering teams.
    - The project status is on track from IS perspective for January 2020 release.
    - Currently tracking the Track2 scoping timeline from Amdocs. Conducting weekly meetings with IS and Engineering 
      teams to look at lab readiness roadmap as well as initial UE samples that may become available for lab and/or field trials.
      Leveraging action-item repository in the enterprise sharepoint site for items we need addressed by IS and Engineering teams.
...
*** Status 
    ...
    *UAT in Progress*
    - A IR0583 – eSIM Consumer Enablement Track 2 - Secondary Device Activation Impact Assessment (IA) is waiting approval (Still Waiting).
    - Reporting, what to report on, was this weeks hot topic.
    - eSIM Consumer Enablement Track 2 document  has been released and ready for review. 
...
*** Go Live Dates
    August release with marketing piece starting first quarter 2020.
*** Possible Billing Issues
...
| IR0411 VoWiFi - Acme             | ON TRACK | IS/Provisioning: Track 1.2a launched in August release. Engineering: A solution to support MMS (multi-media messaging service) and VVM (visual voicemail) is being developed and Acme is working with USCC to accommodate additional time for testing. Acme carrier bundle testing for non-MMS, non-VVM test cases will be completed by 9/30 to submit device logs to Acme.                                                                                                                                                                                                                             | Acme agreed to extend the 9/30 deadline for submitting device logs for MMS/VVM to allow USCC time to develop/test a viable solution. |
...
*** Status
    ...
    b. Inbound (AT&T on USCC network)
       ...
       - Confirmed with AT&T that the specific locations used for whitelisting entire USCC test range also include any MME within that same pool, which provides a lot of different areas to test in (Jody)
       - USCC whitelisted + completed test call + shipped devices to Iowa for the team to begin the deployment (Sherri Jo)
...
*** Current Status
    No updates this week-- Frank is on vacation
...
*** Project Description
   ...
   *Charging Enforcement Function (PCEF)* and *Online Charging System (OCS)* 
   ...
   will deployed in *Santa Clara, Atlanta, Schuamberg and Ashton* starting in May and ending in July.
...
[fn:7] Data Managment and Intergration
