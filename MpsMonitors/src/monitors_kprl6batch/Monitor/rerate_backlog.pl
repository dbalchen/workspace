...
sub checkBacklog{
  ...
  my @conns =(['PRDUSG1','PRDUSG1C','con8usg18'],
  ...
  foreach $one_conn (@conns){
    ...
    my $sql = qq#select cycle_code,cycle_instance,count(*),num_of_rerate_tries from ape1_subscriber_rerate where cycle_instance in (?,?) 
group by cycle_code,cycle_instance,num_of_rerate_tries#;
  ...
  my @sorted_rows = sort {$a->[0] <=> $b->[0] || $a->[1] <=> $b->[1] || $a->[3] <=> $b->[3]} @all_rows;
 # my $count=0;
 # my @sorted_rows=();
 # my @temp=();
 ...
 # }
  ...
  foreach my $row (@sorted_rows){
      ...
      print "{\"cycle_code\":\"" . @$row[0] . "\",\"cycle_instance\":\"" . @$row[1] . "\",\"count\":\"" . @$row[2] . "\",\"retries\":\"" . @$row[3] . "\"}";
    }
