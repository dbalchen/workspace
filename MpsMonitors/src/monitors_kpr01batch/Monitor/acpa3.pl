...
$sql = qq{select nxt_pgm_name, file_status, data_group, count(*) cnt, sum(wr_rec_quantity) rec_quan
          from ac1_control
          where nxt_pgm_name in ('AEM','FILE2E','HLD','LSN','MD','SPL')
          and file_status in ('AE','AF','CN','HO','WA','UD','IU','RD','RJ','RT','UA','US','SP','SM','SF','PR')
          group by nxt_pgm_name, file_status, data_group
         };
...
print '<TABLE CELLPADDING=3 > <TR> <TD ALIGN="CENTER"><B><FONT COLOR="#800040"><a HREF="http://kpr01scdap.uscc.com:8082/WebMonitor/servlet/WebMonitor?MonName=Oncall3'.$EnvName.'" target="Qmon3'.$EnvName.'">'.$EnvName.'</a></FONT></B></TD> </TR> </TABLE>';
