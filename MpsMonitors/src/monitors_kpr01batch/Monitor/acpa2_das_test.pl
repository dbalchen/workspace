...
#-------------------------------------------------------------------------------
 ...
 $sql = qq{select nxt_pgm_name, file_status, count(*) cnt, sum(wr_rec_quantity) rec_quan
           from ac1_control
           where nxt_pgm_name in ('AEM','File2E','HLD','LSN','MD','SPL','RGD')
           and file_status in ('AE','AF','CN','HO','WA','UD','IU','RD','RJ','RT','UA','US','SP','SM','SF','PR','FR')
           having sum(wr_rec_quantity) > 0
           group by nxt_pgm_name, file_status
           order by nxt_pgm_name
          };
 $sth = $dbh->prepare($sql);
 ...
 print '<TABLE CELLPADDING=3 > <TR> <TD ALIGN="CENTER"><B><FONT COLOR="#800040"><a HREF="http://kpr01scdap.uscc.com:8080/WebMonitor/servlet/WebMonitor?MonName='.$EnvName.'" target="Qmon2'.$EnvName.'">'.$EnvName.'</a></FONT></B></TD> </TR> </TABLE>';
