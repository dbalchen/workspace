...
sub save_utilization()
{
  ...
  for my $i (0 .. $#${resultset})  #dereference to get count
  {
    ...
    $rc=$conn->runQuery($seq,"ALL_FILES","duty_cycle","sec",$row->[2],$available_seconds);
    if (!defined $rc)
    {
      $log->print_to_log("binds: $seq,*,duty_cycle,$row->[2],$available_seconds\n");
      croak "\nError in ",__FILE__," runQuery near line ",__LINE__,": \n", $conn->getErrorStr(), "\n\n";
    }
  }
}
