...
=head3 SYNOPSIS
  ...
  perl ProgramBehind.pl MpsLib <path> SqlLib <path> Log <path>
                        threshold <int hours> market <market> 
                        sql_query <file> title <title>
                        [op_job <job>] [op_sid <db instance>]
                        [op_user <username>] [op_pass <password>] 

=head3 DESCRIPTION

  ProgramBehind executable.
  Accepts command line argument list of ProgramBehind properties.
  Formats the arguments into a hash.
  Initiates a ProgramBehind object passing the argument hash to it.
...
=head3 REQUIRED PARAMETER PAIRS
  ...
  threshold - Integer records past which a program is considered behind
              processing.
  
  sql_query - File under SqlLib that contains the ProgramBehind SQL query.
...
BEGIN
{
    ...
    {
        print STDERR "\nUSAGE\n"
          . "perl ProgramBehind.pl MpsLib <path>\n"
          . "                      SqlLib <path>\n"
          . "                      Log <path>\n"
...
my $log       =
  Logger->new(log_dir  => $args{Log},
              log_name => "ProgramBehind_$timestamp.log");
$log->print_to_log($args{title}) or carp "Could not write to log.";

# instantiate RaterStatus object and run_check()
my $program_behind = Monitor::ProgramBehind->new(%args);

eval {
    my $result = $program_behind->run_check();
    if (defined $result)
    {
...
if ($@)
{
    $log->print_to_log("Error in ProgramBehind run_check method.");
    carp "Error in ProgramBehind run_check method.";
}
