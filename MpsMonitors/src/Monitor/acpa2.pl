...
set heading off
set FEEDBACK off
COLUMN PGM                     FORMAT A4
COLUMN FILE_STATUS             FORMAT A2
COLUMN CNT                     FORMAT 99,999
COLUMN RECQ                    FORMAT 9,999,999

select decode(fpfc_nxt_pgm_name,'MAF2MDRV','MAF','MAF2COLL','NMaf','UPS2RCCL','MAF', 'CIBER_RECYCLE','CIBR', 'UPS2COLL','COLL', 'UPS2MDRV','MnDr', 'mpgd_100mn','GUID', 'mpup_100mn','RATE', fpfc_nxt_pgm_name) PGM, file_status, count(*) CNT, sum(wr_rec_quantity) recq from ac_processing_accounting where fpfc_nxt_pgm_name in ('MAF2MDRV','MAF2COLL','UPS2RCCL','CIBER_RECYCLE','UPS2COLL','UPS2MDRV','mpgd_100mn','mpup_100mn') and file_status in ('IU','AF','AE','RD','WA') group by fpfc_nxt_pgm_name, file_status;
exit;
!!";
...
print '<TABLE CELLPADDING=3 > <TR> <TD ALIGN="CENTER"><B><FONT COLOR="#800040"><a HREF="http://chi1pas1.uscc.com:8082/WebMonitor/servlet/WebMonitor?MonName=Oncall'.$EnvName.'" target="Oncall'.$EnvName.'">'.$EnvName.'</a></FONT></B></TD> </TR> </TABLE>';
