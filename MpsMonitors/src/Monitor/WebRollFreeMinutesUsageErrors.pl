...
$hh = "grep 'mpgn_roll_fm: Number of rejected ban(s)' $ENV{'TLG_LOG'}/MPGNROLLFM*".$year.$mon.'*';
...
$hh = "$SQLPlUS -s $DBenv << !!
         set heading off
         set FEEDBACK off
         select error_code, count(*) from usage_error where error_status is not null group by error_code;
         exit;
         !!";
...
for ($a = 0; $a < @stuff; $a++) {
	...
	$hh = "$SQLPlUS -s $DBenv << !!  
    SET HEADING OFF 
    select ERROR_SHORT_DESC from usage_err_handling where ERROR_CODE = '$error_code';
    exit;
    !!";
