...
=head3 SYNOPSIS
  ...
  perl MissingDataFile.pl MpsLib <path> SqlLib <path> Log <path>
...
=head3 DESCRIPTION
  ...
  MissingDataFile executable.
  Accepts command line argument list of MissingDataFile properties.
  Formats the arguments into a hash.
  Initiates a MissingDataFile object passing the argument hash to it.
...
=head3 REQUIRED PARAMETER PAIRS
  ...
  sql_query - File under SqlLib that contains the MissingDataFile SQL query.
...
#print "$a : $ARGV[$a]\n";
    ...
    {
        print STDERR "\nUSAGE\n"
          . "perl MissingDataFile.pl MpsLib <path>\n"
          . "                        SqlLib <path> Log <path>\n"
          . "                        threshold <int hours>\n"
          . "                        market <market>\n"
          . "                        sql_query <file>\n"
          . "                        switch_query <file>\n"
...
my $log       =
  Logger->new(log_dir  => $args{Log},
              log_name => "MissingDataFile_$timestamp.log");
$log->print_to_log($args{title}) or carp "Could not write to log.";

# instantiate MissingDataFile object and run_check()
my $missing_data_file = Monitor::MissingDataFile->new(%args);

eval {
    my $result = $missing_data_file->run_check();
    if (defined $result)
    {
...
if ($@)
{
    $log->print_to_log("Error in MissingDataFile run_check method.");
    carp "Error in MissingDataFile run_check method.";
}
