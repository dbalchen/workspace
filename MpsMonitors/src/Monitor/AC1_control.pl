...
sub dataStatus {
	...
	my $sql =
qq#select count(*),substr(file_name,instr(file_name,'_T')+2,8) "Date",file_status
            from ac1_control
            where identifier = phy_file_ident and substr(file_name,instr(file_name,'_T')+2,4) = '2014'
...
sub fileStatus {
	...
	my $db = DBI->connect( "dbi:Oracle:$dbname", $user, $pass );
	my $sql =
qq#select  file_format, file_status, cur_pgm_name, nxt_pgm_name, host_name, trunc(sys_creation_date) as sys_creation_date, trunc(sys_update_date) as sys_update_date, count(*) from  ac1_control
...
sub getConnectParams {
	...
	$db->{LongReadLen} = 1500;
	my $sql =
	  qq# select conn_params from gn1_connect_params where connect_code=?#;
	my $sth = $db->prepare($sql);
	$sth->execute($connectCode);
	my $params = $sth->fetchrow();
	my $user   = substr(
		$params,
		index( $params, 'USER' ) + 13,
		index( $params, '"', index( $params, 'USER' ) + 13 ) -
		  index( $params, 'USER' ) - 13
	);
	...
	my $pass = substr(
		$params,
		index( $params, 'PASSWORD' ) + 17,
		index( $params, '"', index( $params, 'PASSWORD' ) + 17 ) -
		  index( $params, 'PASSWORD' ) - 17
	);
	...
	my $instance = substr(
		$params,
		index( $params, 'INSTANCE' ) + 17,
		index( $params, '"', index( $params, 'INSTANCE' ) + 17 ) -
		  index( $params, 'INSTANCE' ) - 17
	);
