...
if ($reportVariable{'IN_REC_QUANTITY'} == 0) {$dbconn->disconnect();exit(0);}
...
print RPT $fileId[0]."\t".$fileId[1]."\t".$reportVariable{'IN_REC_QUANTITY'}."\t".$filesum."\t".$reportVariable{'Dropped'}."\t".$reportVariable{'Duplicates'}."\t".$reportVariable{'SenttoTC'}."\t".$reportVariable{'Rejected'}."\t".$rejectSum."\t".$aprm[1]."\t".$aprm[2]."\n";
