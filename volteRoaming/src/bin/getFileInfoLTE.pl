...
$sql = "select /*+ PARALLEL(t1,12) */ count(*), sum(charge_amount),sum(charge_parameter),  from  prm_rom_incol_events_ap t1 where tap_in_file_name = '$argv[0]' group by charge_type";
