...
$sqls{'LTE'} = "select /*+ PARALLEL(t1,12) */ carrier_cd, bp_start_date, count(*), sum(charge_amount), sum(charge_parameter) from prdappc.prm_rom_incol_events t1 where process_date = to_date($ARGV[1],'YYYYMMDD') and carrier_code != 'NLDLT' group by carrier_cd, bp_start_date";
