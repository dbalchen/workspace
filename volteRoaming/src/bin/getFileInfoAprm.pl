...
my $sql = "delete from APRM where DATE_PROCESSED = to_date($ARGV[1],'YYYYMMDD') and USAGE_TYPE like '$ARGV[0]%'";
$sthb = $dbconnb->prepare($sql);
$sthb->execute() or sendErr();

$sql = 'select /*+ PARALLEL(h1,12) */ home_company,carrier_cd, bp_start_date, count(*), sum(usage),sum(TOTAL_CHRG_AMOUNT) from USC_ROAM_EVNTS where (ciber_file_name_1||ciber_file_name_2 like '."'".$filetype."') and generated_rec < 2 group by home_company,carrier_cd, bp_start_date order by carrier_cd";
