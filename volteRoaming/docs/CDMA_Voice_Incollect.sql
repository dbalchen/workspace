Counting Voice events in TC (All these query’s can be done on the ODS)

Get the file and Identifiers for settlement Period (A settlement period is between 16th and 15th of the next month (Ex. April 16th - May 15th)
select file_name, identifier 
    from ac1_control_hist 
    where file_name like 'SDIRI_FCIBER%YYYYMMDD%.DAT'; YYYY = Year, MM = Month DD = Day)
...
Counting events in APRM (Must be done in production)

APRM_SUCCESS (Successfully processed in APRM)
select /*+ PARALLEL(t1,12) */ count(*) 
    from prdappc.usc_roam_evnts t1
...
select /*+ PARALLEL(h1,12) */ carrier_cd, bp_start_date,ciber_file_name_1||ciber_file_name_2, count(*), sum(usage),sum(TOTAL_CHRG_AMOUNT)  
    ...
    where (ciber_file_name_1||ciber_file_name_2 = 'SDIRI_FCIBER_ID000769_T20160608182115.DAT') 
     or   (ciber_file_name_1||ciber_file_name_2 = 'SDIRI_FCIBER_ID000769_T20160608182115.DAT') 
     or   (ciber_file_name_1||ciber_file_name_2 = 'SDIRI_FCIBER_ID000769_T20160608182115.DAT') 
     or   (ciber_file_name_1||ciber_file_name_2 = 'SDIRI_FCIBER_ID000769_T20160608182115.DAT') 
     or   (ciber_file_name_1||ciber_file_name_2 = 'SDIRI_FCIBER_ID000769_T20160608182115.DAT') 
...
select substr(adu,instr(adu,'SDIRI_FCIBER_ID'),41) ciber_file, file_tp, dominant_err_cd, count(*) noe 
    ...
    where prod_id = 2 and event_id <> 2 
     and adu like '%SDIRI_FCIBER_ID000769_T20160608182115.DAT%'
group by substr(adu,instr(adu,'SDIRI_FCIBER_ID'),41), file_tp, dominant_err_cd order by 1,2;
