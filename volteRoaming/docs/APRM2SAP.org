...
** CDMA Data Incollect Settlement
   : select  t1.carrier_cd "Carrier Code", t2.gl_account "GL Account",
   :  sum((t1.tot_chrg_param_val/1024)/1024) "Total Usage Mb", sum(t1.tot_net_usage_chrg) "Total Charges" 
   :   from ic_accumulated_usage t1, usc_sap_extract_v t2 where t1.prod_cat_id = 'in' and t1.bp_start_date = '16-OCT-2017' and t1.future_3 = 'Data'
   :    and t2.au_prod_cat_id = t1.prod_cat_id and t2.au_evt_id = t1.event_id
   :   group by t1.carrier_cd,t2.gl_account order by t1.carrier_cd,t2.gl_account
...
** CDMA Voice Incollect Settlement
   ...
   :    from ic_accumulated_usage t1, usc_sap_extract_v t2 where t1.prod_cat_id = 'IN' and 
...
** CDMA Voice Outcollect Settlement 
   : select  t1.carrier_cd "Carrier Code",  t2.gl_account "Gl Account", t1.rate_plan_cd "Rate Plan",
   :  sum(t1.tot_chrg_param_val) "Total Usage Minutes", 
   :  sum(t1.tot_net_usage_chrg) "Total Charges" 
   :   from ic_accumulated_usage t1,  usc_sap_extract_v t2   
   :    where prod_cat_id = 'RO' and bp_start_date = '16-SEP-2017' and future_3 = 'Voice' 
   :     and t2.au_prod_cat_id = t1.prod_cat_id and t2.au_evt_id = t1.event_id
   :  -- and  rate_plan_cd = 'rprourotax'
   :   group by t1.carrier_cd, t2.gl_account, t1.rate_plan_cd  order by t1.carrier_cd, t2.gl_account, t1.rate_plan_cd 
